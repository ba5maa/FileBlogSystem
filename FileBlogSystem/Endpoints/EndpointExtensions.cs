    ...
    public static class EndpointExtensions
    {
        public static IEndpointRouteBuilder MapApiEndpoints(this IEndpointRouteBuilder app)
        {
            ...
            app.MapPost("/api/auth/login", async (LoginRequest request, IFileContentService contentService, IConfiguration config) =>
            {
                ...
                if (user == null)
                {
                    return Results.Json(new { message = string.Format(EndpointConstants.InvalidUsernameOrPassword), Username = request.Username, Password = request.Password, user = user }, statusCode: 401);
                }
                ...
                {
                    return Results.Json(new { message = string.Format(EndpointConstants.InvalidUsernameOrPassword), Username = request.Username, Password = request.Password, hashedPassword = user.HashedPassword }, statusCode: 401);
                }
